...
const UserRideFeedbackPage = () => {
  const navigate = useNavigate();
  const { state } = useLocation();
  const [searchParams] = useSearchParams();
  const rideId = (state as { rideId?: string } | null)?.rideId || searchParams.get("rideId") || "";
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await backendApi.submitRideFeedback(rideId, rating, comment, token);
      setNotice("Thanks for your feedback. Redirecting...");
      window.setTimeout(() => navigate("/app/home"), 2000);
    } catch (error) {
  ...
  return (
    <div className="relative isolate w-full px-4 min-h-[90vh] flex flex-col pt-10">
      ...
      <div className="flex flex-col items-center justify-center mb-8">
         <div className="w-20 h-20 bg-emerald-50 rounded-full flex flex-col items-center justify-center text-emerald-500 mb-6 shadow-sm">
            <Star size={32} strokeWidth={1.5} className="fill-emerald-500" />
         </div>
         <h1 className="text-3xl font-black text-gray-900 tracking-tight text-center">Rate your Trip</h1>
      ...
      <div className="mt-auto pb-4 pt-8">
         ...
         <button onClick={() => navigate("/app/home")} className="w-full mt-3 h-[60px] rounded-2xl bg-white text-gray-900 font-bold border border-gray-200 shadow-sm disabled:opacity-50 transition-active">
            Skip
         </button>
      </div>
    </div>
...
export default UserRideFeedbackPage;
