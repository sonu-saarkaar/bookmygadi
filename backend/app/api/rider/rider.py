...
def _rider_request_payload(db: Session, ride: Ride) -> RiderRideRequest:
    ...
    # Eagerly load preference if not already loaded
    pref = ride.preference
...
@router.get("/rides", response_model=list[RiderActiveRideRead])
def list_all_driver_rides(
    api_key: RiderAuthType = Depends(_validate_rider_api_key),
    db: Session = Depends(get_db),
) -> list[RiderActiveRideRead]:
    if not api_key.driver_id:
    ...
    rows = (
        db.query(Ride)
        .options(selectinload(Ride.preference))
        .filter(Ride.driver_id == api_key.driver_id)
        .order_by(Ride.created_at.desc())
        .all()
    )
    out: list[RiderActiveRideRead] = []
...
async def accept_rider_request(
    ...
    try:
        from app.schemas import RideRead
        await realtime_manager.broadcast(
            ride.id,
            {"event": "ride_status_updated", "ride": RideRead.model_validate(full_ride).model_dump(mode="json")},
        )
    except Exception:
...
async def update_active_ride_status(
    ...
    response_payload = _active_ride_payload(db, full_ride)
    try:
        from app.schemas import RideRead
        await realtime_manager.broadcast(
            ride.id,
            {
                ...
                "ride": RideRead.model_validate(full_ride).model_dump(mode="json"),
            },
        )
    except Exception:
        pass
