...
@router.post("/search-monitor/auto-assign-stale")
def auto_assign_stale_searches(
    threshold_seconds: int = Query(default=120, ge=30, le=1800),
    limit: int = Query(default=20, ge=1, le=100),
    admin: User = Depends(get_admin_user),
    db: Session = Depends(get_db),
) -> dict:
    rows = (
        db.query(RideSearchEvent)
        .filter(RideSearchEvent.status == "searching")
        .order_by(RideSearchEvent.search_started_at.asc())
        .limit(limit * 5)
        .all()
    )
    drivers = db.query(User).filter(User.role == "driver", User.driver_status == "approved").order_by(User.updated_at.desc()).all()
    assigned = 0
    now = datetime.utcnow()
    ...
    for row in rows:
        if assigned >= limit:
            break
        elapsed = max(0, int((now - (row.search_started_at or row.created_at)).total_seconds()))
        if elapsed < threshold_seconds:
            continue
        if not drivers:
            break
        candidate = drivers[assigned % len(drivers)]
        row.assigned_driver_id = candidate.id
        row.status = "assigned"
        row.searched_seconds = elapsed
        if row.ride_id:
            ride = db.get(Ride, row.ride_id)
            if ride and (ride.status or "").lower() in {"pending", "searching", "assigned"}:
                ride.driver_id = candidate.id
                ride.status = "accepted"
                if ride.accepted_at is None:
                    ride.accepted_at = now
        assigned += 1

    _audit(db, admin, "dispatch", f"auto-assigned stale searches count={assigned} threshold={threshold_seconds}s")
    db.commit()
    return {"ok": True, "assigned": assigned, "threshold_seconds": threshold_seconds}
