...
MAX_SPEED_KMH = 180.0
ACTIVE_DRIVER_RIDE_STATUSES = frozenset({"accepted", "arriving", "in_progress"})


def _vehicle_type_matches(reg_vt: str, ride_vt: str) -> bool:
    a, b = (reg_vt or "car").lower(), (ride_vt or "car").lower()
    if a == b:
        return True
    pickup_family = {"bolero", "pickup", "mini_truck", "goods"}
    lorry_family = {"lorry", "truck", "tractor", "tempo", "canter"}
    if b in pickup_family and a in pickup_family | {"car"}:
        return True
    if b in lorry_family and a in lorry_family:
        return True
    return False
...
def _driver_tokens_prioritized(db: Session, ride: Ride) -> list[str]:
    ...
    def active_ride_count(driver_id: str) -> int:
        return (
            db.query(Ride)
            .filter(Ride.driver_id == driver_id, Ride.status.in_(ACTIVE_DRIVER_RIDE_STATUSES))
            .count()
        )

    def nearest_km(driver_id: str) -> float:
        ...
        last = (
            ...
            .filter(
                ...
                Ride.driver_live_lat.isnot(None),
                Ride.driver_live_lng.isnot(None),
            )
            .order_by(Ride.updated_at.desc())
            .first()
        )
        if not last or last.driver_live_lat is None or last.driver_live_lng is None:
            return 999.0
        dk = _distance_km(
            ride.pickup_lat,
            ride.pickup_lng,
            last.driver_live_lat,
            last.driver_live_lng,
        )
        return float(dk) if dk is not None else 999.0

    def sort_key(d: User) -> tuple:
        pref = 0 if d.id in preferred_ids else 1
        near_km = nearest_km(d.id)
        in_radius = 0 if near_km <= settings.dispatch_prefer_radius_km else 1
        busy = active_ride_count(d.id)
        return (pref, in_radius, busy, near_km)

    ordered = sorted(drivers, key=sort_key)
    return [d.fcm_token for d in ordered if d.fcm_token]
...
async def create_ride(
    payload: RideCreate,
    current_user: User = Depends(require_role(["customer", "admin", "driver", "rider"])),
    db: Session = Depends(get_db),
) -> RideRead:
    reserve_distance = _distance_km(payload.pickup_lat, payload.pickup_lng, payload.destination_lat, payload.destination_lng)
    if (
        payload.requested_fare is not None
        and payload.estimated_fare_min is not None
        and payload.requested_fare < payload.estimated_fare_min
    ):
        raise HTTPException(status_code=400, detail="Requested fare cannot be below estimated minimum")
    if (
        payload.requested_fare is not None
        and payload.estimated_fare_max is not None
        and payload.requested_fare > payload.estimated_fare_max
    ):
        raise HTTPException(status_code=400, detail="Requested fare cannot exceed estimated maximum")
    ...
    db.add(
        RideSearchEvent(
            ...
            search_mode="reserve" if payload.preference and (payload.preference.urgency_type or "").lower() == "reserve" else "instant",
    ...
    nearby_driver_tokens = _driver_tokens_prioritized(db, ride)
...
@router.post("/{ride_id}/driver-location", response_model=RideTrackingRead)
async def update_driver_location_compat(
    ride_id: str,
    payload: LocationUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> RideTrackingRead:
    """Native/web clients using Bearer driver JWT (same path family as customer-location)."""
    ride = db.get(Ride, ride_id)
    if not ride:
        raise HTTPException(status_code=404, detail="Ride not found")
    if current_user.role not in {"driver", "admin"}:
        raise HTTPException(status_code=403, detail="Only driver can update this location")
    ...
    accuracy, observed_at = _validate_location_payload(ride, "driver", payload)
    ...
    await realtime_manager.broadcast(ride.id, return_payload_dict)
    return return_payload
