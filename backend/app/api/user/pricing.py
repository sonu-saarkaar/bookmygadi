...
from app.core.instant_pricing import (
    compute_instant_quote,
    normalize_service_profile,
    reserve_row_price_multiplier,
)
...
def get_quote(
    ...
    service_profile: str | None = Query(
        default=None,
        description="Optional: car|bike|auto|pickup|lorry|general|events|farming — overrides inference from vehicle_type",
    ),
    use_full_distance: bool = Query(
        default=False,
        description="If true, fare uses full GPS distance (outstation-style); else local cap from settings.",
    ),
    ...
    dist = _distance_km(pickup_lat, pickup_lng, destination_lat, destination_lng) or float(base_km)
    demand = _demand_multiplier()

    q = compute_instant_quote(
        vehicle_type=vehicle_type,
        service_profile=service_profile,
        raw_distance_km=float(dist),
        route_rule=route_rule,
        vehicle_mod=vehicle_mod,
        demand=demand,
        local_cap_km=settings.local_instant_pricing_cap_km,
        use_full_distance=use_full_distance,
    )
    ...
    return PricingQuoteRead(
        ...
        vehicle_type=vehicle_type.lower(),
        estimated_distance_km=round(float(dist), 2),
        billable_distance_km=round(q.billable_distance_km, 2),
        suggested_fare=q.suggested_fare,
        min_fare=q.min_fare,
        max_fare=q.max_fare,
        demand_multiplier=round(q.demand_multiplier, 2),
        vehicle_multiplier=round(q.vehicle_multiplier, 2),
        pricing_algorithm=q.algorithm,
        service_profile=normalize_service_profile(vehicle_type, service_profile),
    )
...
def get_reserve_quote(
    ...
    pickup_lng: float | None = Query(default=None),
    vehicle_type: str = Query(default="car"),
    reserve_category: str | None = Query(
        default=None,
        description="general | events | farming — adjusts quoted driver prices for category",
    ),
    ...
    for p in prices:
        ...
        mult = reserve_row_price_multiplier(vehicle_type, reserve_category)
        base_price = int(round(_price_for_duration(p, duration_hours) * mult))
        rows.append(
            ReserveQuoteRow(
