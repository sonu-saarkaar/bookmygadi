...
class BroadcastCampaign(Base, TimestampMixin):
    ...
    target_audience: Mapped[str] = mapped_column(String(40)) # all, users, riders
    channel: Mapped[str] = mapped_column(String(20), default="push") # push, sms, email
    status: Mapped[str] = mapped_column(String(20), default="sent") # sent, failed
    sent_count: Mapped[int] = mapped_column(Integer, default=0)
    created_by: Mapped[str | None] = mapped_column(String(120), nullable=True)

class ReferralSettings(Base, TimestampMixin):
    __tablename__ = "referral_settings"
    
    id: Mapped[str] = mapped_column(String, primary_key=True, default="global_config")
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    signup_bonus: Mapped[int] = mapped_column(Integer, default=50)
    referrer_bonus: Mapped[int] = mapped_column(Integer, default=100)
    max_referrals_per_user: Mapped[int] = mapped_column(Integer, default=10)
