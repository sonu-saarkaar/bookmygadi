...
const ICON_OPTIONS = ["Car", "Bike", "Truck", "Bus", "Package", "Zap", "Star", "Shield", "MapPin"];
const COLOR_OPTIONS = [
...
export const ServiceManagementBoard = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const rows = await adminV2Api.listAllServices();
      setServices(rows || []);
    } catch {
      try {
        const fallbackRows = await backendApi.listServices();
        setServices(fallbackRows || []);
        pushToast("Loaded public services only. Admin list failed.", "warning");
      } catch {
  ...
  const handleOpenEdit = (row: any) => {
    ...
    setFormData({
      ...
      tag_highlight: row.tag_highlight || "",
      icon_name: row.icon_name || "Car",
      color_scheme: row.color_scheme || "from-emerald-400 to-emerald-600",
      service_mode: row.service_mode || "Instant Ride",
  ...
  const handleSave = async () => {
    if (!formData.title.trim()) {
      pushToast("Service title is required", "warning");
      return;
    }
    if (!formData.description.trim()) {
      pushToast("Service description is required", "warning");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        pushToast("Service created", "success");
      }
      setIsModalOpen(false);
      await loadData();
    } catch (e: any) {
      pushToast(e?.message || "Unable to save service", "danger");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      pushToast(e?.message || "Failed to delete service", "danger");
    }
  };

  const activeServices = useMemo(
    () => services.filter((s) => (s.service_mode || "").toLowerCase() === viewType.toLowerCase()),
    [services, viewType],
  );

  return (
    <div className="relative isolate w-full px-2 min-h-[90vh] pb-24 max-w-5xl mx-auto">
      <div className="py-2 px-1 mb-6">
        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Service Management</h1>
        <p className="text-sm font-medium text-gray-500 mt-1">Website me dikhne wale services load aur edit karein</p>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 relative z-10">
                    ...
                    <p className="text-[11px] font-semibold text-slate-500 uppercase">Vehicle: {item.vehicle_type} | Model: {item.vehicle_model || "-"}</p>
                  </div>
                  <button onClick={() => handleOpenEdit(item)} className="w-full h-14 rounded-2xl bg-gray-900 text-white shadow-lg text-center font-black text-sm uppercase tracking-[0.18em] hover:bg-black transition-colors flex items-center justify-center gap-2 relative z-10 active:scale-[0.98]">
          ...
          <div className="p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500 font-bold" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Description</label>
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500" rows={2} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tag</label>
                  <input type="text" value={formData.tag_highlight} onChange={(e) => setFormData({ ...formData, tag_highlight: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Vehicle Type</label>
                  <input type="text" value={formData.vehicle_type} onChange={(e) => setFormData({ ...formData, vehicle_type: e.target.value.toLowerCase() })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500 lowercase" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Vehicle Model</label>
                  <input type="text" value={formData.vehicle_model} onChange={(e) => setFormData({ ...formData, vehicle_model: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Service Mode</label>
                  <select value={formData.service_mode} onChange={(e) => setFormData({ ...formData, service_mode: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500 bg-white">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={formData.color_scheme} onChange={(e) => setFormData({ ...formData, color_scheme: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500 bg-white">
                    {COLOR_OPTIONS.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Icon</label>
                  <select value={formData.icon_name} onChange={(e) => setFormData({ ...formData, icon_name: e.target.value })} className="w-full border border-slate-300 rounded-xl p-3 outline-none focus:border-indigo-500 bg-white">
